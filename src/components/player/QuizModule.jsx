...
export default function QuizModule({ quiz, enrollment, userId, onAchievement }) {
  ...
  const handleSubmit = async () => {
    ...
    if (enrollment) {
      ...
      scores.push({ quiz_id: quiz.id, score: correct, max_score: quiz.questions.length, submitted_at: new Date().toISOString() });
    ...
    await supabase.from("analytics_events").insert({
      ...
      event_type: "quiz_submit", meta: { score: s, correct, total: quiz.questions.length },
    });
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6">
      ...
      {submitted && (
        <div className={`p-4 rounded-xl mb-5 ${score >= 70 ? "bg-emerald-50 border border-emerald-200" : "bg-orange-50 border border-orange-200"}`}>
          <div className="flex items-center gap-2 mb-1">{score >= 70 && <Trophy className="w-4 h-4 text-emerald-600" />}<p className={`text-sm font-semibold ${score >= 70 ? "text-emerald-700" : "text-orange-700"}`}>Score: {score}% ({quiz.questions.filter((q, i) => answers[i] === q.correct_index).length}/{quiz.questions.length})</p></div>
      ...
      {!submitted && (<Button onClick={handleSubmit} disabled={Object.keys(answers).length < quiz.questions.length} className="mt-5 bg-[#00a98d] hover:bg-[#008f77] text-white rounded-xl">Submit Quiz</Button>)}
    </div>
  );
}
