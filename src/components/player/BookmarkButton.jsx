...
export default function BookmarkButton({ user, courseId, lectureId, currentTime = 0 }) {
  ...
  const handleSave = () => {
    ...
    supabase.from("bookmarks").insert({
      ...
      timestamp_seconds: Math.round(currentTime), note,
    }).then(({ error }) => {
      if (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className={`gap-2 rounded-xl ${saved ? "text-emerald-600 border-emerald-200" : ""}`}>
      ...
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} className="bg-[#00a98d] hover:bg-[#008f77] text-white">Save Bookmark</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
