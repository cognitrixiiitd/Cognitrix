...
const changeTypeColors = {
  ...
  lecture_added: "bg-emerald-50 text-emerald-700",
  lecture_deleted: "bg-red-50 text-red-700",
  quiz_generated: "bg-indigo-50 text-indigo-700",
};
...
export default function CourseHistory({ courseId }) {
  const { data: revisions = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      base44.entities.CourseRevision.filter(
        { course_id: courseId },
        "-created_date",
      ),
    enabled: !!courseId,
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <Badge
                    className={`text-[10px] ${changeTypeColors[revision.change_type] || "bg-gray-50 text-gray-700"}`}
                  >
                    {revision.change_type.replace(/_/g, " ")}
                  </Badge>
                ...
                <div className="flex items-center gap-3 text-xs text-gray-400">
                  <span className="flex items-center gap-1">
                    ...
                    {moment(revision.created_date).fromNow()}
                  </span>
                  <span className="flex items-center gap-1">
