...
export default function ProfessorDashboardMetrics({ professorId }) {
  ...
  const { data: questions = [] } = useQuery({
    ...
    queryFn: async () => { if (!courseIds.length) return []; const { data } = await supabase.from("questions").select("id, course_id, user_name, text, created_at").in("course_id", courseIds).order("created_at", { ascending: false }).limit(100); return data || []; },
    enabled: courseIds.length > 0,
  });
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title="Questions Asked" value={questions.length} icon={MessageSquare} color="orange" />
      </div>
