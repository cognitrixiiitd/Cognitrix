...
export default function CoursePlayer() {
  ...
  useEffect(() => {
    const initUser = async () => {
      ...
      if (stats.length === 0) {
        await base44.entities.StudentStats.create({
          user_id: me.id,
          total_points: 0,
          level: 1,
        });
  ...
  const { data: course, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const c = await base44.entities.Course.filter({ id: courseId });
      return c[0];
    },
    enabled: !!courseId,
  });
  ...
  const markCompleteMutation = useMutation({
    mutationFn: async (lectureId) => {
      ...
      const watchDuration = watchTimeStart
        ? Math.round((Date.now() - watchTimeStart) / 1000)
        : 0;
      ...
      await base44.entities.Enrollment.update(enrollment.id, {
        ...
        time_spent_minutes:
          (enrollment.time_spent_minutes || 0) + Math.round(watchDuration / 60),
      ...
      if (!wasAlreadyCompleted) {
        ...
        if (progress === 100) {
          const stats2 = await base44.entities.StudentStats.filter({
            user_id: user.id,
          });
          if (stats2[0]) {
            const newCoursesCompleted = (stats2[0].courses_completed || 0) + 1;
            await base44.entities.StudentStats.update(stats2[0].id, {
              courses_completed: newCoursesCompleted,
              total_points: (stats2[0].total_points || 0) + 100,
            });
            ...
            const achievement = await base44.entities.Achievement.create({
              ...
              course_id: courseId,
              achievement_type:
                newCoursesCompleted === 1 ? "first_course" : "course_completed",
              badge_name:
              ...
              badge_description:
                newCoursesCompleted === 1
                  ...
                  : `Completed ${newCoursesCompleted} courses`,
              badge_icon: "trophy",
              points_awarded: 100,
            });
    ...
    mutationFn: async ({ fromTime, toTime }) => {
      ...
      if (seekBack && Math.abs(toTime - fromTime) > 3) {
        await base44.entities.AnalyticsEvent.create({
          ...
          event_type: "pause",
          timestamp_seconds: toTime,
          meta: {
            seek_from: fromTime,
            seek_to: toTime,
            replay_seconds: Math.abs(toTime - fromTime),
            possible_confusion: true,
          },
        });
      }
    },
  ...
  const flagStuckMutation = useMutation({
    mutationFn: async () => {
      ...
      await base44.entities.Question.create({
        ...
        course_id: courseId,
        lecture_id: currentLecture.id,
        text: `Flagged as stuck on: ${currentLecture.title}`,
        status: "open",
        is_stuck_flag: true,
      });
      await base44.entities.AnalyticsEvent.create({
  ...
  const currentLecture = lectures[currentLectureIndex];
  const isCompleted =
    enrollment?.completed_lectures?.includes(currentLecture?.id) ||
    justCompletedId === currentLecture?.id;
    ...
    <div className="-m-4 lg:-m-8">
      ...
      <div className="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-400">
              {currentLectureIndex + 1} of {lectures.length} lectures
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => flagStuckMutation.mutate()}
            disabled={flagStuckMutation.isPending}
            className="text-xs gap-1 text-orange-500 hover:text-orange-600 hover:bg-orange-50"
          >
      ...
      >
            ...
            {currentLecture ? (
              <div>
                ...
                currentLecture.type === "video" ? (
                  ...
                  >
                    {currentLecture.type === "youtube" &&
                    getYouTubeEmbedUrl(currentLecture.source_url) ? (
                      <iframe
                        src={getYouTubeEmbedUrl(currentLecture.source_url)}
                  ...
                  currentLecture.type === "slides" ? (
                  <div className="bg-gray-100 rounded-2xl p-8 text-center">
                    <FileText className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                    <p className="text-sm text-gray-500">Document content</p>
                    {currentLecture.source_url && (
                      <a
                        href={currentLecture.source_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-sm text-[#00a98d] hover:underline mt-2 inline-block"
                      >
                        Open Document
                      </a>
                    )}
                ...
                <div className="mt-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      {currentLecture.ai_generated_description && (
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          !isCompleted &&
                          markCompleteMutation.mutate(currentLecture.id)
                        }
                        disabled={isCompleted || markCompleteMutation.isPending}
                        className={`rounded-xl text-sm gap-1.5 ${!isCompleted ? "bg-[#00a98d] hover:bg-[#008f77] text-white" : "text-emerald-600 border-emerald-200"}`}
                      >
                  ...
                  {currentLecture.suggested_resources?.length > 0 && (
                    <div className="mt-4 p-4 bg-blue-50 rounded-xl border border-blue-100">
                      ...
                      <div className="space-y-2">
                        {currentLecture.suggested_resources.map((res, i) => (
                          <a
                            key={i}
                            href={res.url}
                            target="_blank"
                ...
                <LectureRecommendations
                  ...
                  onSelectLecture={setCurrentLectureIndex}
                />
                ...
                {/* Quiz - Only show after marking complete */}
                ...
                {currentQuiz && !isCompleted && (
                  <div className="mt-6 p-6 bg-amber-50 rounded-2xl border border-amber-200 text-center">
                    <p className="text-sm text-amber-800 font-medium">
                      📝 Quiz available after completing this lecture
                    </p>
                    <p className="text-xs text-amber-600 mt-1">
                      Mark the lecture as complete to unlock the quiz
                    </p>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100">
                  <Button
                    ...
                    onClick={() =>
                      setCurrentLectureIndex(
                        Math.max(0, currentLectureIndex - 1),
                      )
                    }
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setCurrentLectureIndex(
                        Math.min(lectures.length - 1, currentLectureIndex + 1),
                      )
                    }
        ...
        <PlayerSidebar
          ...
          onSelect={setCurrentLectureIndex}
          completedLectures={enrollment?.completed_lectures || []}
          showQA={showQA}
          courseId={courseId}
          lectureId={currentLecture?.id}
          user={user}
