...
export default function MyLearning() {
  ...
  const unenrollMutation = useMutation({
    mutationFn: async (enrollmentId) => {
      // Using .select() tells Supabase to return the row if it was successfully deleted
      const { data, error } = await supabase.from("enrollments").delete().eq("id", enrollmentId).select();
      if (error) throw error;
      if (!data || data.length === 0) {
        throw new Error("Deletion was blocked by the database (Row Level Security missing an allowed DELETE policy for students).");
      }
      return data;
    },
    ...
    onError: (err, _v, ctx) => { 
      queryClient.setQueryData(["my-enrollments", user?.id], ctx.prev);
      window.alert(err.message || "Failed to remove course");
    },
    onSuccess: () => {
      window.alert("Course has been removed from your learning.");
    },
        ...
        <div className="flex gap-2 mt-3">
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button size="sm" variant="ghost" className="text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg">
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Are you sure you want to remove this course from your learning? This action cannot be undone and your progress will be permanently deleted.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={() => unenrollMutation.mutate(enrollment.id)} className="bg-red-600 text-white hover:bg-red-700">
                  Remove
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
