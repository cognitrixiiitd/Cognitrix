  ...
  const { data: questions = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      base44.entities.Question.filter(
        { user_id: user.id, is_stuck_flag: false },
        "-created_date",
      ),
    enabled: !!user?.id,
  });
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Track your questions and answers from courses
        </p>
      </div>
      ...
      {questions.length === 0 ? (
        <EmptyState
          ...
          description="Ask questions during your course lectures and they'll appear here."
        />
      ) : (
              ...
              <div className="flex items-start gap-3">
                <div
                  className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    q.status === "answered" ? "bg-emerald-50" : "bg-yellow-50"
                  }`}
                >
                  {q.status === "answered" ? (
