...
import {
  Users, BookOpen, TrendingUp, Flag, BarChart3,
  Download, Clock, Trophy
} from "lucide-react";
...
export default function Analytics() {
  ...
  const { data: enrollments = [], isLoading: loadingEnrollments } = useQuery({
    queryKey: ["analytics-enrollments", courses.map(c => c.id).join(",")],
    queryFn: async () => {
      const all = [];
      for (const c of courses) {
        const e = await base44.entities.Enrollment.filter({ course_id: c.id });
        all.push(...e);
      }
      return all;
    },
  ...
  const { data: stuckFlags = [] } = useQuery({
    queryKey: ["analytics-stuck", courses.map(c => c.id).join(",")],
    queryFn: async () => {
      const all = [];
      for (const c of courses) {
        const q = await base44.entities.Question.filter({ course_id: c.id, is_stuck_flag: true });
        all.push(...q);
      }
      return all;
    },
  ...
  const { data: lectures = [] } = useQuery({
    queryKey: ["analytics-lectures", courses.map(c => c.id).join(",")],
    queryFn: async () => {
      const all = [];
      for (const c of courses) {
        const l = await base44.entities.Lecture.filter({ course_id: c.id });
        all.push(...l);
      }
      return all;
    },
  ...
  const { data: questions = [] } = useQuery({
    queryKey: ["analytics-questions", courses.map(c => c.id).join(",")],
    queryFn: async () => {
      const all = [];
      for (const c of courses) {
        const q = await base44.entities.Question.filter({ course_id: c.id, is_stuck_flag: false });
        all.push(...q);
      }
      return all;
    },
  ...
  if (loadingCourses || loadingEnrollments) return <LoadingSpinner />;
  ...
  const avgCompletion = filteredEnrollments.length > 0
    ? Math.round(filteredEnrollments.reduce((s, e) => s + (e.progress_percent || 0), 0) / filteredEnrollments.length)
    : 0;
  const completedCount = filteredEnrollments.filter(e => e.status === "completed").length;
  const totalTimeHours = Math.round(filteredEnrollments.reduce((s, e) => s + (e.time_spent_minutes || 0), 0) / 60);
  ...
  const avgQuizScore = (() => {
    const allScores = filteredEnrollments.flatMap(e => e.quiz_scores || []);
    if (allScores.length === 0) return 0;
    return Math.round(allScores.reduce((s, q) => s + ((q.score / q.max_score) * 100), 0) / allScores.length);
  })();
  ...
  const exportCSV = () => {
    ...
    const rows = filteredEnrollments.map(e => {
      ...
      const avgScore = e.quiz_scores?.length > 0
        ? Math.round(e.quiz_scores.reduce((s, q) => s + ((q.score / q.max_score) * 100), 0) / e.quiz_scores.length) + "%"
        : "N/A";
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          <Select value={selectedCourse} onValueChange={setSelectedCourse}>
            <SelectTrigger className="w-56 rounded-xl border-gray-200 text-sm">
          ...
          <Button variant="outline" onClick={exportCSV} className="rounded-xl gap-2 text-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          {courseEnrollmentData.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={courseEnrollmentData}>
                ...
                <Tooltip />
                <Bar dataKey="students" fill="#00a98d" radius={[4, 4, 0, 0]} />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6">
          ...
          {completionDistribution.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie data={completionDistribution} cx="50%" cy="50%" innerRadius={60} outerRadius={100} dataKey="value" label={({ name, percent }) => `${name} (${(percent * 100).toFixed(0)}%)`}>
                  {completionDistribution.map((_, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <EngagementInsights 
          lectures={lectures.filter(l => 
            selectedCourse === "all" || l.course_id === selectedCourse
          )} 
          stuckFlags={stuckFlags.filter(f => 
            selectedCourse === "all" || f.course_id === selectedCourse
          )}
        ...
        <SentimentAnalysis 
          questions={questions.filter(q => 
            selectedCourse === "all" || q.course_id === selectedCourse
          )} 
        />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          <h3 className="text-sm font-semibold text-black">Student Details</h3>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-50">
                ...
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Course</th>
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Progress</th>
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Status</th>
                <th className="text-left text-xs font-medium text-gray-500 px-6 py-3">Avg Quiz</th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={e.id} className="border-b border-gray-50 hover:bg-gray-50">
                    <td className="px-6 py-3">
                      <p className="text-sm font-medium text-black">{e.student_name || "Student"}</p>
                    ...
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-500">{e.progress_percent || 0}%</span>
                      </div>
                    </td>
                    <td className="px-6 py-3">
              ...
              {filteredEnrollments.length === 0 && (
                <tr>
                  <td colSpan={5} className="text-center py-8 text-sm text-gray-400">No enrollment data</td>
                </tr>
              )}
            </tbody>
          </table>
