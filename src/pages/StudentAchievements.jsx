...
export default function StudentAchievements() {
  ...
  const { data: stats, isLoading: loadingStats } = useQuery({
    ...
    queryFn: async () => {
      const s = await base44.entities.StudentStats.filter({ user_id: user.id });
      return s[0] || null;
    },
    enabled: !!user,
  });
  ...
  const { data: achievements = [] } = useQuery({
    ...
    queryFn: () =>
      base44.entities.Achievement.filter({ user_id: user.id }, "-created_date"),
    enabled: !!user,
  });
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-gradient-to-br from-purple-50 to-white rounded-2xl border border-purple-100 p-5">
          ...
          <div className="mt-3">
            ...
            <p className="text-xs text-purple-600 mt-1">
              {nextLevelPoints} pts to Level {(stats?.level || 1) + 1}
            </p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-amber-50 to-white rounded-2xl border border-amber-100 p-5">
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-xs font-medium text-amber-700">Total Points</p>
          </div>
      ...
      <Tabs defaultValue="achievements" className="space-y-6">
        <TabsList className="bg-gray-100 rounded-xl">
          <TabsTrigger value="achievements" className="rounded-lg text-xs">
            My Badges
          </TabsTrigger>
